...
export function PaymentDetailSheet() {
  ...
  return (
    <Sheet open onClose={() => set({ paymentDetailIdx: null })}>
      ...
      <p className="m-0 text-[28px] font-extrabold text-navy mb-4">{row.amount}</p>

      <div className="bg-cream rounded-2xl px-4 py-3.5 mb-3">
        <p className="m-0 text-[11px] font-extrabold uppercase tracking-wider mb-2.5" style={{ color: '#A39B8B' }}>
          Breakdown
        </p>
        {DUES_LEGEND.map((d) => (
          <div key={d.label} className="flex items-center justify-between py-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[13px] font-semibold text-navy">{d.label}</span>
            </div>
